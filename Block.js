...
var Block = function (gl) {
	...
	if (this.blockData.color == "red") {
		this.points = [
			vec2(this.startx + (minSize * this.blockData.size), 1.01 + (minSize * this.blockData.size) * 3 / 4),
			vec2(this.startx + (minSize * this.blockData.size), 1.01 + (minSize * this.blockData.size) / 3),
			vec2(this.startx + (minSize * this.blockData.size) / 2, 1.01),
			vec2(this.startx, 1.01 + (minSize * this.blockData.size) / 3),
			vec2(this.startx, 1.01 + (minSize * this.blockData.size) * 3 / 4),

		];
	} else if (this.color == "blue") {
		this.points = [
			vec2(this.startx, 1.01 + (minSize * this.blockData.size)),
			vec2(this.startx + (minSize * this.blockData.size), 1.01 + (minSize * this.blockData.size)),
			vec2(this.startx + (minSize * this.blockData.size), 1.01),
			vec2(this.startx, 1.01),

		];
	} else {
		this.points = [
			vec2(this.startx + (minSize * this.blockData.size) * 2 / 3, 1.01 + (minSize * this.blockData.size) * 3 / 4),
			vec2(this.startx + (minSize * this.blockData.size), 1.01 + (minSize * this.blockData.size) / 2),
			vec2(this.startx + (minSize * this.blockData.size) / 2, 1.01),
			vec2(this.startx, 1.01 + (minSize * this.blockData.size) / 2),
			vec2(this.startx + (minSize * this.blockData.size) / 3, 1.01 + (minSize * this.blockData.size) * 3 / 4),
